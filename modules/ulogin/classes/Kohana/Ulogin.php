...
class Kohana_Ulogin {
	...
	{	
                $fields = array();
                foreach(array_merge($this->config['username'], $this->config['fields']) AS $i => $v)
                {
                    $v = is_numeric($i) ? $v : $i;
                    if(!in_array($v, $fields)) $fields[] = $v;
                }
                $optional = array();
                //print_r($this->config['optional']);
                foreach($this->config['optional'] AS $i => $v)
                {
                    $v = is_numeric($i) ? $v : $i;
                    if(!in_array($v, $optional)) $optional[] = $v;
                }
		$params = 	
	...
	public function login()
	{
		...
		$user = json_decode($s, true);
                ...
                if(!empty($user['error'])) throw new Kohana_Exception($user['error']);
		...
		if (!$ulogin->loaded())
		{
			...
			else
			{
				...
				$data['username'] = text::translate_to_url(trim($data['username']));
				...
				foreach($cfg_fields as $field => $field2)
				{
					if (!empty($user[is_numeric($field)?$field2:$field]))
						$data[$field2] = $user[is_numeric($field)?$field2:$field];
				}
