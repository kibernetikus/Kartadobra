...
class Controller_Ajax_Public extends Controller_Template_Ajax {
    ...
    public function action_registration()
    {
        if ($_POST)
        {
            $data = Validation::factory($_POST)
                ...
                ->rule('fam', 'not_empty')
                ->rule('fam', 'min_length', array(':value', 4))
                ->rule('fam', 'max_length', array(':value', 32))
                    ->label('fam', 'Название организации')
                ->rule('e', 'not_empty')
                ->rule('e', 'email')
            ...
            if ($data->check())
            {
                ...
                $u->last_name = $_POST['fam'];
                $u->phone = $_POST['phone'];
                $u->skype = $_POST['skype'];
                $u->social = $_POST['social'];
                $u->city_id = $city->id;
    ...
    public function action_orgregistration()
    {
        if ($_POST)
        {
            $data = Validation::factory($_POST)
