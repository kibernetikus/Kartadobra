<div class="thumbnail">
    <?if($prev->loaded()){?><a href="/offers/<?=$prev->id?>" class="h" style="text-decoration:none;"><i class=" icon-arrow-left"></i> Предыдущее</a><?}?>
    <span style="margin-left:100px;"><a href="/offers<?=$my?'/my':''?>" class="h" style="text-decoration:none;"> <i class="icon-arrow-up"></i>К списку заданий</a></span>
    <span style="margin-left:90px;"><?if($next->loaded()){?><a href="/offers/<?=$next->id?>" class="h" style="text-decoration:none;">Следующее <i class=" icon-arrow-right"></i></a><?}?> </span>
    ...
    <div class="caption">
        ...
        <?if($t->mode==0){?>
        <div id="notclose">
        <h4><?=isset($count)?'Предложение выполнено <span id="cnt_users">'.text::get_word_int($count, 'rz', TRUE).'</span>':'Просьба пока не выполнена'?></h4>
